...
using SimpleConsole;
using EventHubPartitionReceiver.Configuration;
...
{
    /// <summary>
    /// Simple tool to receive events from a specific Event Hub partition.
    /// </summary>
    /// <param name="partition">Partition identifier to receive events from.</param>
    ...
    static async Task<int> Main(string partition, int time, bool debug)
    {
        ...
        ConsoleEx.WriteLine(ConsoleColor.Yellow, partition);
        ...
        var receiver = new EventHubPartitionReceiver(config.ConnectionString, eventHubName, config.ConsumerGroup, partition, config.StorageConnectionString, config.StorageContainerName, logger);
